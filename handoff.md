...
### 幸運格擴充至 6 種獎勵
1. **新增 3 種幸運獎勵**：🌟幸運色金色、🎵唱一首歌、🍙飯糰
2. **飯糰機率調低**：加權池（其他各3份，飯糰1份，共16份），飯糰機率 = 其他選項的 1/3
3. **luckyQueue**：從 `buildSession()` 以加權陣列 `luckyWeighted` 洗牌產生，`luckyQueueIdx` 依序取值
4. **金色特效**：金色背景 + ✨🌟⭐ 星星展示
5. **唱歌畫面**：紫色音符 🎵，「唱完才能繼續冒險喔！」
6. **飯糰畫面**：顯示寵物 + 飯糰圖示 + 現有顆數 + 「餵飯糰！」按鈕（含升級邏輯）

### 圈圈叉叉格（⭕ tictactoe）
7. **完整實作** `triggerTicTacToe()` — 3×3 棋盤，玩家先手（O）
8. **音效**：O 高頻正弦波（900→1200Hz），X 低頻三角波（450→280Hz）
9. **AI 勝率設計**：40% 最佳步 + 60% 隨機步，玩家約有 60% 勝率
10. **結果文字**：贏→「你贏了！送你一個微笑，開心一整天 😊」／輸→「不要氣餒，再接再厲！💪」／平→「平手！勢均力敵！🤝」

### 繪圖格改進
11. **標題兩層**：小字「Please draw a」＋大字「a rabbit 🐰 🔊」（移除中文）
12. **答案選項**：改為英文單字（非中文/emoji）
13. **獎勵改為水果**：蛋糕 → 🍇葡萄 / 🍓草莓 / 🍊橘子（隨機三選一），文字「得到健康水果！」
14. **備用題庫**：無單字庫時顯示 Cat / Dog / Rabbit（fallback wordlist）
15. **畫布放大**：`320×340`，modal `maxHeight:82vh`
16. **餵食唱歌畫面簡化**：移除「謝謝你的水果」標題和水果展示

### 格子池重新設定
17. **新格子數量**：挑戰9、複習9、幸運2、傳送2、運動4、占卜2、笑話2、圈圈叉叉3、繪圖3 = 共 **36 格隨機**（+4固定學習格，總計 40 格）
18. **無連續打亂演算法**：強制置放（超過剩餘一半時優先放） + 5輪後處理 swap

### 遊戲說明更新
19. **L1 說明**：改為「每10格學習2個英文單字」
20. **寵物養成說明**：正確門檻 0→3→9→18，星等每10顆升一星
21. **新增格子說明**：圈圈叉叉格、繪圖格

### 關鍵函式（新增）
| 函式 | 說明 |
|------|------|
| `triggerTicTacToe()` | 圈圈叉叉遊戲完整邏輯 |
| `triggerLucky()` | 幸運格（現有6種獎勵） |
| `_getDrawableWord()` | 取繪圖用單字，含 Cat/Dog/Rabbit 備用 |
